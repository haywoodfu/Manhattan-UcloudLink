...
- (BOOL)open {
    ...
    //weiliang---2013-8-7  sqlite set SQLITE_CONFIG_SINGLETHREAD mode for thread safe
#if SQLITE_VERSION_NUMBER >= 3007000
    sqlite3_shutdown();
    //    NSLog(@"sqlite3 lib version: %s", sqlite3_libversion());
    int errConfig = sqlite3_config(SQLITE_CONFIG_MULTITHREAD);
    if (errConfig == SQLITE_OK) {
        //        NSLog(@"Can now use sqlite on multiple threads, using the same connection");
    } else {
        NSLog(@"setting sqlite thread safe mode to serialized failed!!! return code: %d", errConfig);
    }
#endif
